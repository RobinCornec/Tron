...
public class Player1Control : MonoBehaviour {
	...
	void Update () {
		turn = Mathf.Lerp (turn, turnSpeed * Input.GetAxis ("HorizontalP1"), Time.deltaTime * 1f);
		transform.RotateAround (rotationPivot.position, Vector3.up, turn);
		move = Vector3.Lerp(move,Vector3.forward * speed * Input.GetAxis("VerticalP1"),Time.deltaTime *1f);
		move.y -= 5;
